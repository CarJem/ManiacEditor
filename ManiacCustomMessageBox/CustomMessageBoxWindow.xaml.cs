...
namespace WPFCustomMessageBox
{
	...
	internal partial class CustomMessageBoxWindow : Window
	{
		...
		internal CustomMessageBoxWindow(string message)
		{
			...
			InitializeComponent();
			if (CustomMessageBox.UseDarkTheme) SetDarkTheme();
		...
		private void DisplayImage(MessageBoxImage image)
		{
			...
			switch (image)
			{
				case MessageBoxImage.Exclamation:       // Enumeration value 48 - also covers "Warning"
					icon.BeginInit();
					icon.UriSource = new Uri("pack://application:,,,/Resources/Monitors/WarningStatic2.gif");
					icon.EndInit();
					break;
				case MessageBoxImage.Error:             // Enumeration value 16, also covers "Hand" and "Stop"
					icon.BeginInit();
					icon.UriSource = new Uri("pack://application:,,,/Resources/Monitors/ErrorStatic2.gif");
					icon.EndInit();
					break;
		...
		{
			BackgroundGrid.Background = new SolidColorBrush(System.Windows.Media.Color.FromArgb(40,40,40,40));
			ButtonBar.Background = new SolidColorBrush(System.Windows.Media.Color.FromArgb(70, 70, 70, 70));
		}
	}
}
