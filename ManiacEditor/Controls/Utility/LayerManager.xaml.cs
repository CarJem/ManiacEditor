...
namespace ManiacEditor.Controls.Utility
{
	...
	public partial class LayerManager : Window
	{
		...
		{
			var current = Layers.ElementAt(GetSelectedLayer()).HorizontalLayeRules.ElementAt(GetSelectedLayerScroll()).LinesMapList.ElementAt(GetSelectedHorizontalMap()) as Classes.Editor.Scene.Sets.EditorLayer.ScrollInfoLines;
			if (null == current) return;
