...
namespace ManiacEditor
{
    public class EditorInGame
    {
        ...
        public void DrawSinglePlayer(DevicePanel d, int playerID)
        {
            ...
            if (playerID == EditClasses.EditorState.PlayerBeingTracked) Editor.GoToPosition(x, y);
            ...
            if (GameRunning == false && attachMode)
            {
                ...
                new Thread(() =>
                {
                    Editor.Dispatcher.Invoke(new Action(() =>
                    {
                        ...
                        // Swap the Scene
                        if (EditClasses.EditorState.LevelID != -1)
                        {
                            Editor.GameMemory.WriteByte(CurrentScene_ptr[GameVersion.IndexOf(SelectedGameVersion)], (byte)EditClasses.EditorState.LevelID);
                            // Restart the Scene
        ...
        public void MoveThePlayerToHere()
        {
            if (Editor.InGame.GameRunning)
            {
                ...
                Editor.GameMemory.WriteInt16(ObjectAddress + 2, (short)(EditClasses.EditorState.LastX / EditClasses.EditorState.Zoom));
                Editor.GameMemory.WriteInt16(ObjectAddress + 6, (short)(EditClasses.EditorState.LastY / EditClasses.EditorState.Zoom));
            }
        }

        public void SetPlayerRespawnToHere()
        {
            Point clicked_point = new Point((int)(EditClasses.EditorState.LastX / EditClasses.EditorState.Zoom), (int)(EditClasses.EditorState.LastY / EditClasses.EditorState.Zoom));
            if (GameRunning)
            {
        ...
        public void TrackthePlayer(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            if (item != null)
            {
                ...
                if (!item.IsChecked)
                {
                    ...
                    EditClasses.EditorState.PlayerBeingTracked = player;
                }
                else
                {
                    item.IsChecked = false;
                    EditClasses.EditorState.PlayerBeingTracked = -1;
                }
