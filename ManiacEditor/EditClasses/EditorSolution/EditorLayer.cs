...
namespace ManiacEditor.EditClasses.Solution
{
    public class EditorLayer : IDrawable
    {
        ...
        public void SelectAll()
        {
            for (int y = 0; y < _layer.Tiles.Length; y += 1)
            {
                for (int x = 0; x < _layer.Tiles[y].Length; x += 1)
                {
                    ...
                    else if (_layer.Tiles[y][x] == 0xffff && EditClasses.EditorState.CopyAir)
                    {
                        SelectedTiles.Add(new Point(x, y));
                        RefreshTileCount();
                    }
        ...
        public void Select(Point point, bool addSelection = false, bool deselectIfSelected = false)
        {
            if (!addSelection) Deselect();
            point = new Point(point.X / EditorConstants.TILE_SIZE, point.Y / EditorConstants.TILE_SIZE);
            EditClasses.EditorState.SelectedTileX = point.X;
            EditClasses.EditorState.SelectedTileY = point.Y;
            if (point.X >= 0 && point.Y >= 0 && point.X < this._layer.Tiles[0].Length && point.Y < this._layer.Tiles.Length)
            {
                ...
                else if (this._layer.Tiles[point.Y][point.X] != 0xffff || EditClasses.EditorState.CopyAir)
                {
                    // Just add the point
                    SelectedTiles.Add(point);
                    RefreshTileCount();
        ...
        public void TempSelection(Rectangle area, bool deselectIfSelected)
        {
            ...
            for (int y = Math.Max(area.Y / EditorConstants.TILE_SIZE, 0); y < Math.Min(DivideRoundUp(area.Y + area.Height, EditorConstants.TILE_SIZE), _layer.Height); ++y)
            {
                for (int x = Math.Max(area.X / EditorConstants.TILE_SIZE, 0); x < Math.Min(DivideRoundUp(area.X + area.Width, EditorConstants.TILE_SIZE), _layer.Width); ++x)
                {
                    if (SelectedTiles.Contains(new Point(x, y)) || (_layer.Tiles[y][x] != 0xffff || EditClasses.EditorState.CopyAir))
                    {
        ...
        public void Deselect()
        {
            ...
            SelectedTiles.Clear();
            SelectedTiles.Values.Clear();
            EditClasses.EditorState.SelectedTilesCount = 0;
        }
        ...
        public bool HasTileAt(Point point)
        {
            ...
            if (point.X >= 0 && point.Y >= 0 && point.X < this._layer.Tiles[0].Length && point.Y < this._layer.Tiles.Length)
            {
                return (_layer.Tiles[point.Y][point.X] != 0xffff || EditClasses.EditorState.CopyAir);
            }
            return false;
        }
        ...
        public void OldDrawTile(Graphics g, ushort tile, int x, int y)
        {
            ...
            g.DrawImage(CurrentSolution.CurrentTiles.StageTiles.Image.GetBitmap(new Rectangle(0, TileIndex * EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE), flipX, flipY),
            ...
            if (EditClasses.EditorState.ShowCollisionA)
            {
            ...
            if (EditClasses.EditorState.ShowCollisionB)
            {
            ...
            if (EditClasses.EditorState.ShowFlippedTileHelper == true)
            {
                g.DrawImage(CurrentSolution.CurrentTiles.StageTiles.EditorImage.GetBitmap(new Rectangle(0, 3 * EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE), false, false),
                            new Rectangle(x * EditorConstants.TILE_SIZE, y * EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE));
            }
            if (EditClasses.EditorState.ShowTileID == true)
            {
                g.DrawImage(CurrentSolution.CurrentTiles.StageTiles.IDImage.GetBitmap(new Rectangle(0, TileIndex * EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE), false, false),
                            new Rectangle(x * EditorConstants.TILE_SIZE, y * EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE));
            }
            ...
            {
                Bitmap Map;
                if (drawA) Map = CurrentSolution.CurrentTiles.StageTiles.CollisionMaskA.GetBitmap(new Rectangle(0, (tile & 0x3ff) * EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE), flipX, flipY);
                else Map = CurrentSolution.CurrentTiles.StageTiles.CollisionMaskB.GetBitmap(new Rectangle(0, (tile & 0x3ff) * EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE), flipX, flipY);
        ...
        public void Draw(DevicePanel d)
        {
            ...
            if (EditorInstance.EditLayerA != null && (EditorInstance.EditLayerA != this && EditorInstance.EditLayerB != this))
                Transperncy = 0x32;
            else if (EditorInstance.EditorToolbar.EditEntities.IsCheckedAll && EditorInstance.EditLayerA == null && EditClasses.EditorState.ApplyEditEntitiesTransparency)
                Transperncy = 0x32;
            else
                Transperncy = 0xFF;
        ...
        private bool isChunkSelected(int _x, int _y)
        {
            ...
            int mouse_x = (int)EditClasses.EditorState.LastX;
            int mouse_y = (int)EditClasses.EditorState.LastY;
        ...
        public void DrawTile(Graphics g, ushort tile, int x, int y, bool isSelected = false)
        {
            ...
            void DrawCollision(bool drawA, System.Drawing.Color colour, bool flipX, bool flipY)
            {
                ...
                if (drawA) collisionMap = CurrentSolution.CurrentTiles.StageTiles.CollisionMaskA.GetBitmap(new Rectangle(0, (tile & 0x3ff) * EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE), flipX, flipY);
                else collisionMap = CurrentSolution.CurrentTiles.StageTiles.CollisionMaskB.GetBitmap(new Rectangle(0, (tile & 0x3ff) * EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE, EditorConstants.TILE_SIZE), flipX, flipY);
        ...
        public int ProcessParallax(ManiacEditor.HorizontalLayerScroll layer, int maxWidth, int speed = 1, int duration = 0)
        {
            ...
            // Playback
            if (EditClasses.EditorState.ParallaxAnimationChecked && EditClasses.EditorState.AllowAnimations)
            {
        ...
        public void RefreshTileCount()
        {
            ...
            EditClasses.EditorState.DeselectTilesCount = TempSelectionDeselectTiles.Count;
            EditClasses.EditorState.SelectedTilesCount = GlobalSelectedTiles - EditClasses.EditorState.DeselectTilesCount;
        }
