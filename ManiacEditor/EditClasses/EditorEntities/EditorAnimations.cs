...
namespace ManiacEditor
{
    ...
    public class EditorAnimations
    {
        ...
        {
            // Playback
            if (EditorInstance.EditorToolbar.ShowAnimations.IsChecked.Value && EditClasses.EditorState.AllowSpriteAnimations)
            {
				if (speed > 0)
                {
        ...
        public void ProcessMovingPlatform(int angleDefault, UInt32 speed = 3)
        {
            ...
            int duration = 1;
            // Playback
            if (EditorInstance.EditorToolbar.ShowAnimations.IsChecked.Value && EditClasses.EditorState.AllowMovingPlatformAnimations)
            {
				if (speed > 0)
                {
        ...
        public int[] ProcessMovingPlatform2D(int ampX, int ampY, int x, int y, int width, int height, UInt32 speed = 1)
        {
            ...
            // Playback || I disabled anything with both x and y values because they have way too many issues atm
            if (Editor.Instance.ShowAnimations.Checked && Properties.EditClasses.EditorState.Default.movingPlatformsChecked)
            {
                if (speed > 0)
                {
                    int speed1 = (int)speed * 64 / (duration == 0 ? 256 : duration);
