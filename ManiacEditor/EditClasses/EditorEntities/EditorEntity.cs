...
namespace ManiacEditor
{
    ...
    public class EditorEntity : IDrawable
	{
        ...
        public void SetSelected(bool value)
        {
            ...
            CurrentSolution.Entities.UpdateSelectedIndexForEntities();
        }
        ...
        public void Move(Point diff, bool relative = true)
        {
            ...
            if (Editor.Instance.InGame.GameRunning && Settings.MyGameOptions.RealTimeObjectMovementMode && !IsInternalObject)
            {
                ...
                int ObjectAddress = ObjectStart + (ObjectSize * CurrentSolution.Entities.GetRealSlotID(_entity));
        ...
        public Rectangle GetDimensions(bool GridAlignment = false)
        {
            ...
            if (GridAlignment)
            {
                ...
                {
                    int x = EditClasses.EditorState.MagnetSize * (_entity.Position.X.High / EditClasses.EditorState.MagnetSize);
                    int y = EditClasses.EditorState.MagnetSize * (_entity.Position.Y.High / EditClasses.EditorState.MagnetSize);
                    return new Rectangle(x, y, EditClasses.EditorState.MagnetSize, EditClasses.EditorState.MagnetSize);
                }
                else
                {
        ...
        public bool SetFilter()
        {
            ...
            if (EditClasses.EditorState.entitiesTextFilter != "" && !_entity.Object.Name.Name.Contains(EditClasses.EditorState.entitiesTextFilter))
            {
                filteredOut = true;
            }
        ...
        {
            if (filteredOut) return;
            if (EditorEntityDrawing.RenderingSettings.LinkedObjectsToRender.Contains(_entity.Object.Name.Name) && EditClasses.EditorState.ShowEntityPathArrows)
            {
                try
                {
        ...
        public virtual void DrawBase(GraphicsHandler d, bool drawSelectionBox = true)
        {
			...
			if (!drawSelectionBoxInFront && !EditClasses.EditorState.EntitySelectionBoxesAlwaysPrioritized && drawSelectionBox) DrawSelectionBox(d, x, y, Transparency, color, color2);
        ...
        public virtual void FallbackDraw(GraphicsHandler d, int x, int y, int _ChildX, int _ChildY, int Transparency, System.Drawing.Color color, bool overridePosition = false)
        {
            ...
            if (editorAnim != null && editorAnim.Frames.Count > 0)
            {
                ...
                { // No frame to render
                    if (EditClasses.EditorState.ShowEntitySelectionBoxes) d.DrawRectangle(x, y, x + EditorConstants.ENTITY_NAME_BOX_WIDTH, y + EditorConstants.ENTITY_NAME_BOX_HEIGHT, System.Drawing.Color.FromArgb(Transparency, color));
                }
                //Failsafe?
                //DrawOthers(d);
        ...
        {
            if (EditClasses.EditorState.ShowEntitySelectionBoxes && !useOtherSelectionVisiblityMethod && this.IsObjectOnScreen(d))
            {
                if (renderNotFound)
                {
        ...
        {
            // Playback
            if (EditClasses.EditorState.AllowSpriteAnimations)
            {
                if (speed > 0)
                {
