    ...
    public class UIKeyBinder : EntityRenderer
    {
        public override void Draw(Structures.EntityRenderProp properties)
        {
            ...
            if (editorAnim != null && editorAnim.Frames.Count != 0)
            {
                ...
                d.DrawBitmap(new Classes.Editor.Draw.GraphicsHandler.GraphicsInfo(frame), x, y + frame.Frame.PivotY,
                    frame.Frame.Width, frame.Frame.Height, false, Transparency);
            }
            if (editorAnimKey != null && editorAnimKey.Frames.Count != 0)
            {
                var frame = editorAnimKey.Frames[Animation.index];
                d.DrawBitmap(new Classes.Editor.Draw.GraphicsHandler.GraphicsInfo(frame), x + frame.Frame.PivotX - 16, y + frame.Frame.PivotY,
                    frame.Frame.Width, frame.Frame.Height, false, Transparency);
            }
