    ...
    public class EncoreRoute : EntityRenderer
    {
        ...
        public override void Draw(GraphicsHandler d, SceneEntity entity, Classes.Editor.Scene.Sets.EditorEntity e, int x, int y, int Transparency, int index = 0, int previousChildCount = 0, int platformAngle = 0, EditorAnimations Animation = null, bool selected = false, AttributeValidater attribMap = null)
        {
            if (Classes.Editor.Solution.CurrentScene?.Scratch != null)
            {
                ...
                var editorAnim = Interfaces.Base.MainEditor.Instance.EntityDrawing.LoadAnimation2("EditorIcons2", d.DevicePanel, 0, 7, fliph, flipv, false);

                if (editorAnim != null && editorAnim.Frames.Count != 0)
                {
                    //Draw the Encore Route Tiles
                    DrawTileGroup(d, x / 16, y / 16, x2, y2, height, width, Transparency, entity, Interfaces.Base.MainEditor.Instance);
        ...
        public void DrawTileGroup(GraphicsHandler d, int x, int y, int x2, int y2, int height, int width, int Transperncy, SceneEntity entity, Interfaces.Base.MainEditor EditorInstance)
        {
        ...
        public void DrawTile(GraphicsHandler d, ushort tile, int x, int y, bool selected, int Transperncy, Interfaces.Base.MainEditor EditorInstance)
        {
