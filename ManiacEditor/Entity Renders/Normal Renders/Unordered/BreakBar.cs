...
namespace ManiacEditor.Entity_Renders
{
    public class BreakBar : EntityRenderer
    {
        ...
        public override void Draw(GraphicsHandler d, SceneEntity entity, Classes.Editor.Scene.Sets.EditorEntity e, int x, int y, int Transparency, int index = 0, int previousChildCount = 0, int platformAngle = 0, EditorAnimations Animation = null, bool selected = false, AttributeValidater attribMap = null)
        {
            ...
            var editorAnim = Editor.Instance.EntityDrawing.LoadAnimation2("BreakBar", d.DevicePanel, orientation, -1, false, false, false);
            if (editorAnim != null && editorAnim.Frames.Count != 0)
            {
                ...
                for (int i = -length / 2; i <= length / 2; ++i)
                {
                    ...
                    d.DrawBitmap(new GraphicsHandler.GraphicsInfo(frameBase), x + frameBase.Frame.PivotX + (orientation == 1 ? i * frameBase.Frame.Width : 0),
                ...
                d.DrawBitmap(new GraphicsHandler.GraphicsInfo(frameTop), x + frameTop.Frame.PivotX - (orientation == 1 ? (length / 2 + 1) * frameBase.Frame.Width : 0) + (orientation == 1 ? 4 : 0),
                ...
                d.DrawBitmap(new GraphicsHandler.GraphicsInfo(frameBottom), x + frameBottom.Frame.PivotX + (orientation == 1 ? (length / 2 + 1) * frameBase.Frame.Width : 0) - (orientation == 1 ? 4 : 0),
