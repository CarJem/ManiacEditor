...
namespace ManiacEditor
{
    public class EditorFindReplace
    {
        ...
        public void EditorTileReplaceTest(int findValue, int replaceValue, int applyState, bool copyResults, bool perserveColllision)
        {
            ...
            {
                Classes.Edit.Solution.EditLayerA.Select(new Rectangle(0, 0, 32768, 32768), true, false);
                Editor.UI.UpdateEditLayerActions();
                Dictionary<Point, ushort> copyData = Classes.Edit.Solution.EditLayerA.CopyToClipboard(true);
                ...
                {
                    Classes.Edit.Solution.EditLayerA.PasteFromClipboard(new Point(0, 0), Editor.FindReplaceClipboard);
                    Editor.UI.UpdateEditLayerActions();
                }
                Editor.UI.UpdateEditLayerActions();
                Editor.FindReplaceClipboard.Clear();
        ...
        public void EditorTileFind(int tile, int applyState, bool copyResults)
        {
            ...
            for (int y = 0; y < Classes.Edit.Solution.EditLayerA.Layer.Height; y++)
            {
                for (int x = 0; x < Classes.Edit.Solution.EditLayerA.Layer.Width; x++)
                {
                    ushort TileIndex = (ushort)(Classes.Edit.Solution.EditLayerA.Layer.Tiles[y][x] & 0x3ff); //What is our tile index?
            ...
            for (int y = 0; y < Classes.Edit.Solution.EditLayerA.Layer.Height; y++)
            {
                for (int x = 0; x < Classes.Edit.Solution.EditLayerA.Layer.Width; x++)
                {
                    ...
                    if (TileIndex == FindTile) //do the tiles match?
                    {
                        ...
                        Classes.Edit.Solution.EditLayerA.Layer.Tiles[y][x] = Tile; //Set our new tile Value
