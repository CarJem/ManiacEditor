...
namespace ManiacEditor
{
    public class EditorFindReplace
    {
        ...
        public void FindUnusedTiles()
        {
            ...
            Editor.UI.ToggleEditorButtons(false);
        ...
        {
            IEnumerable<Classes.Edit.Scene.EditorSolution.EditorLayer> AllLayers = Classes.Edit.Scene.EditorSolution.CurrentScene.AllLayers;
            bool unused = true;
        ...
        public void EditorTileReplaceTest(int findValue, int replaceValue, int applyState, bool copyResults, bool perserveColllision)
        {
            if (Editor.IsTilesEdit())
            {
                ...
                Dictionary<Point, ushort> copyData = Editor.EditLayerA.CopyToClipboard(true);
                ...
                {
                    Editor.EditLayerA.PasteFromClipboard(new Point(0, 0), Editor.FindReplaceClipboard);
                    Editor.UI.UpdateEditLayerActions();
                }
                Editor.UI.UpdateEditLayerActions();
                Editor.FindReplaceClipboard.Clear();
        ...
        public void EditorTileFind(int tile, int applyState, bool copyResults)
        {
            ...
            for (int y = 0; y < Editor.EditLayerA.Layer.Height; y++)
            {
                for (int x = 0; x < Editor.EditLayerA.Layer.Width; x++)
                {
                    ushort TileIndex = (ushort)(Editor.EditLayerA.Layer.Tiles[y][x] & 0x3ff); //What is our tile index?
            ...
            for (int y = 0; y < Editor.EditLayerA.Layer.Height; y++)
            {
                for (int x = 0; x < Editor.EditLayerA.Layer.Width; x++)
                {
                    ...
                    if (TileIndex == FindTile) //do the tiles match?
                    {
                        ...
                        Editor.EditLayerA.Layer.Tiles[y][x] = Tile; //Set our new tile Value
