...
namespace ManiacEditor.Classes.Scene.Sets
{
    [Serializable]
    public class EditorEntity : IDrawable
	{
        ...
        {
            DrawBase(new Methods.Draw.GraphicsHandler(g), Editor);
        }

        public void Draw(Graphics g)
        {
            DrawBase(new Methods.Draw.GraphicsHandler(g));
        }

        public Classes.Scene.Sets.EditorEntity GetSelf()
        {
            return this;
        }
        ...
        public void DrawUIButtonBack(Methods.Draw.GraphicsHandler d, int x, int y, int width, int height, int frameH, int frameW, int Transparency)
        {
            width += 1;
            height += 1;
            ...
            int zoomOffset = (Methods.Editor.SolutionState.Zoom % 1 == 0 ? 0 : 1);
        ...
        public void DrawTriangle(Methods.Draw.GraphicsHandler d, int x, int y, int width, int height, int frameH, int frameW, int state = 0, int Transparency = 0xFF)
        {
        ...
        public void Move(Point diff, bool relative = true)
        {
            ...
            if (Methods.Runtime.GameHandler.GameRunning && ManiacEditor.Properties.Settings.MyGameOptions.RealTimeObjectMovementMode && !IsInternalObject)
            {
                ...
                int ObjectAddress = ObjectStart + (ObjectSize * Methods.Editor.Solution.Entities.GetRealSlotID(_entity));
        ...
        public System.Drawing.Color GetBoxInsideColor()
        {
            if (InTempSelection)
            {
                return (TempSelected && ManiacEditor.Methods.Editor.SolutionState.IsEntitiesEdit()) ? System.Drawing.Color.MediumPurple : System.Drawing.Color.MediumTurquoise;
            }
            else
            {
                return (Selected && ManiacEditor.Methods.Editor.SolutionState.IsEntitiesEdit()) ? System.Drawing.Color.MediumPurple : System.Drawing.Color.MediumTurquoise;
            }
        }
		...
		public virtual void DrawBoxOnly(DevicePanel d)
		{
            ...
            DrawSelectionBox(new Methods.Draw.GraphicsHandler(d), x, y, Transparency, color, color2);

		}
        public virtual void Draw(DevicePanel d)
        {
            Draw(new Methods.Draw.GraphicsHandler(d));
        }

        public virtual void Draw(Methods.Draw.GraphicsHandler d)
        {
            if (filteredOut) return;
            if (Methods.Entities.EntityDrawing.RenderingSettings.LinkedObjectsToRender.Contains(_entity.Object.Name.Name) && Methods.Editor.SolutionState.ShowEntityPathArrows)
            {
                try
                {
        ...
        public virtual void DrawInternal(DevicePanel d)
        {
            ...
            int x = _entity.Position.X.High;
            int y = _entity.Position.Y.High;
            DrawSelectionBox(new Methods.Draw.GraphicsHandler(d), x, y, Transparency, System.Drawing.Color.Transparent, System.Drawing.Color.Red);
        }

        public virtual void DrawBase(Methods.Draw.GraphicsHandler d, bool drawSelectionBox = true)
        {
            List<string> entityRenderList = Methods.Entities.EntityDrawing.RenderingSettings.ObjectToRender;
            List<string> onScreenExlusionList = (ManiacEditor.Properties.Settings.MyPerformance.DisableRendererExclusions ? new List<string>() : Methods.Entities.EntityDrawing.RenderingSettings.ObjectCullingExclusions);
			...
			if (!drawSelectionBoxInFront && !Methods.Editor.SolutionState.EntitySelectionBoxesAlwaysPrioritized && drawSelectionBox) DrawSelectionBox(d, x, y, Transparency, color, color2);
        ...
        public virtual void PrimaryDraw(Methods.Draw.GraphicsHandler d, List<string> onScreenExlusionList)
        {
            if ((this.IsObjectOnScreen(d) || onScreenExlusionList.Contains(_entity.Object.Name.Name)) && ManiacEditor.Properties.Settings.MyPerformance.UseAlternativeRenderingMode)
            {
            ...
            else if (!ManiacEditor.Properties.Settings.MyPerformance.UseAlternativeRenderingMode)
            {
                ManiacEditor.Controls.Editor.MainEditor.Instance.EntityDrawing.DrawOthers(d, _entity, this, childX, childY, index, previousChildCount, platformAngle, EditorAnimations, Selected, childDrawAddMode);
            }
        }
        public virtual void FallbackDraw(Methods.Draw.GraphicsHandler d, int x, int y, int _ChildX, int _ChildY, int Transparency, System.Drawing.Color color, bool overridePosition = false)
        {
            int __X = GetChildX();
            int __Y = GetChildY();
            if (overridePosition)
            ...
            if (editorAnim != null && editorAnim.Frames.Count > 0)
            {
                ...
                if (frame != null)
                {
                    ...
                    d.DrawBitmap(new Methods.Draw.GraphicsHandler.GraphicsInfo(frame.Texture), __X + frame.Frame.PivotX + ((int)offset.X * frame.Frame.Width), __Y + frame.Frame.PivotY + ((int)offset.Y * frame.Frame.Height),
                        frame.Frame.Width, frame.Frame.Height, false, Transparency);
                }
                else
                { // No frame to render
                    if (Methods.Editor.SolutionState.ShowEntitySelectionBoxes) d.DrawRectangle(x, y, x + Methods.Editor.EditorConstants.ENTITY_NAME_BOX_WIDTH, y + Methods.Editor.EditorConstants.ENTITY_NAME_BOX_HEIGHT, System.Drawing.Color.FromArgb(Transparency, color));
                }
                //Failsafe?
                //DrawOthers(d);
        ...
        public void DrawSelectionBox(Methods.Draw.GraphicsHandler d, int x, int y, int Transparency, System.Drawing.Color color, System.Drawing.Color color2)
        {
            if (Methods.Editor.SolutionState.ShowEntitySelectionBoxes && !useOtherSelectionVisiblityMethod && this.IsObjectOnScreen(d))
            {
                if (renderNotFound)
                {
                    d.DrawRectangle(x, y, x + Methods.Editor.EditorConstants.ENTITY_NAME_BOX_WIDTH, y + Methods.Editor.EditorConstants.ENTITY_NAME_BOX_HEIGHT, System.Drawing.Color.FromArgb(Transparency, color));
                }
                else
                {
                    d.DrawRectangle(x, y, x + Methods.Editor.EditorConstants.ENTITY_NAME_BOX_WIDTH, y + Methods.Editor.EditorConstants.ENTITY_NAME_BOX_HEIGHT, GetSelectedColor(color2));
                }
                ...
                if (ManiacEditor.Properties.Settings.MyPerformance.DisableEntitySelectionBoxText == false)
                {
                    ...
                    {
                        d.DrawTextSmall(string.Format("{0} (ID: {1})", _entity.Object.Name, _entity.SlotID), x + 2, y + 2, Methods.Editor.EditorConstants.ENTITY_NAME_BOX_WIDTH - 4, System.Drawing.Color.FromArgb(Transparency, System.Drawing.Color.Black), true);
                    }
                }

                if (SelectedIndex != -1)
                { 
                    d.DrawText(string.Format("{0}", SelectedIndex + 1), x + 1, y + 1, Methods.Editor.EditorConstants.ENTITY_NAME_BOX_WIDTH, System.Drawing.Color.Black, true);
                    d.DrawText(string.Format("{0}", SelectedIndex + 1), x, y, Methods.Editor.EditorConstants.ENTITY_NAME_BOX_WIDTH, System.Drawing.Color.Red, true);
                }
            }
        }
        ...
        public System.Drawing.Color GetSelectedColor(System.Drawing.Color color)
        {
            if (InTempSelection)
            {
                return System.Drawing.Color.FromArgb(TempSelected && ManiacEditor.Methods.Editor.SolutionState.IsEntitiesEdit() ? 0x60 : 0x00, color);
            }
            else
            {
                return System.Drawing.Color.FromArgb(Selected && ManiacEditor.Methods.Editor.SolutionState.IsEntitiesEdit() ? 0x60 : 0x00, color);
            }
        }
        ...
        public bool IsObjectOnScreen(Methods.Draw.GraphicsHandler d)
        {
        ...
        {
            // Playback
            if (Methods.Editor.SolutionState.AllowSpriteAnimations)
            {
                if (speed > 0)
                {
