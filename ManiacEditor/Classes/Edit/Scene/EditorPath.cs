...
namespace ManiacEditor.Classes.Edit.Scene
{
    public class EditorPath
	{
		...
		public bool SetGameConfig(string DataDirectory)
		{
			try
			{
				Classes.Edit.Scene.Solution.GameConfig = new Gameconfig(Path.Combine(DataDirectory, "Game", "GameConfig.bin"));
			...
			catch
			{
				...
				try
				{
                    Classes.Edit.Scene.Solution.GameConfig = new Gameconfig(Path.Combine(DataDirectory, "Game", "GameConfig_ME.bin"));
		...
		public bool SetTileConfig(string configPath)
		{
			try
			{
                ...
                {
                    Classes.Edit.Scene.Solution.TileConfig = new RSDKv5.Tileconfig(Path.Combine(configPath, "Stages", CurrentZone, "TileConfig.bin"));
        ...
        public bool SetTileConfigFromFilePath(string filepath)
        {
            try
            {
                Classes.Edit.Scene.Solution.TileConfig = new RSDKv5.Tileconfig(Path.Combine(filepath, "TileConfig.bin"));
		...
		public bool SetStageTiles(string tilePath, string Zone, string colors = null)
		{
			try
			{
				Classes.Edit.Scene.Solution.CurrentTiles.StageTiles = new StageTiles(Path.Combine(tilePath, "Stages", CurrentZone), colors);
        ...
        public bool SetStageTilesFromFilePath(string filePath, string colors = null)
        {
            try
            {
                Classes.Edit.Scene.Solution.CurrentTiles.StageTiles = new StageTiles(Path.Combine(filePath), colors);
		...
		public bool SetStageConfig(string configPath)
		{
			try
			{
				Classes.Edit.Scene.Solution.StageConfig = new StageConfig(Path.Combine(configPath, "Stages", CurrentZone, "StageConfig.bin"));
        ...
        public bool SetStageConfigFromFilePath(string filepath)
        {
            try
            {
                Classes.Edit.Scene.Solution.StageConfig = new StageConfig(Path.Combine(filepath, "StageConfig.bin"));
		...
		public Stamps GetEditorStamps(string Zone)
		{
            ...
            Stamps_Source = Path.Combine(SceneFile_Directory, Classes.Edit.Scene.Solution.CurrentScene.EditorMetadata.StampName.Replace("\0", ""));
            if (IsEditorStampsValid())
            {
                return new Stamps(Path.Combine(SceneFile_Directory, Classes.Edit.Scene.Solution.CurrentScene.EditorMetadata.StampName.Replace("\0", "")));
            }
            else
            {
        ...
        {
            Stamps_SourceID = SceneFile_SourceID;
            Classes.Edit.Scene.Solution.CurrentScene.EditorMetadata.StampName = Name;
        }

        public bool IsEditorStampsValid()
		{
			return File.Exists(Path.Combine(SceneFile_Directory, Classes.Edit.Scene.Solution.CurrentScene.EditorMetadata.StampName.Replace("\0", "")));
		}
		#endregion
